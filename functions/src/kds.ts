...
function json(res: Response, status: number, body: JsonObject) {
  cors(res);
...
function isRecentIso(iso: string | null | undefined, ttlMs: number) {
  if (!iso) return false;
...
function sortByIsoAsc<T>(rows: T[]): T[] {
  return [...rows].sort((left, right) =>
    asTrimmedString((left as { created_at?: string | null }).created_at).localeCompare(
      asTrimmedString((right as { created_at?: string | null }).created_at)
    )
  );
}

function sortByIsoDesc<T>(rows: T[]): T[] {
  return [...rows].sort((left, right) =>
    asTrimmedString((right as { created_at?: string | null }).created_at).localeCompare(
      asTrimmedString((left as { created_at?: string | null }).created_at)
    )
  );
}

function getTenantIdFromSubcollectionDoc(
  snap:
    | FirebaseFirestore.QueryDocumentSnapshot<FirebaseFirestore.DocumentData>
    | FirebaseFirestore.DocumentSnapshot<FirebaseFirestore.DocumentData>
) {
...
function sanitizeKdsDevice(id: string, tenantId: string, data: Record<string, unknown>) {
...
function isDeletedKdsDeviceData(data: Record<string, unknown>) {
  return !!asNullableString(data.deleted_at);
}

async function requireTenantAccess(req: Request, res: Response, tenantId: string) {
...
function normalizeStationType(value: unknown) {
...
function normalizeStationIcon(value: unknown) {
  return asTrimmedString(value) || "ChefHat";
}

function normalizeStationColor(value: unknown) {
  return asTrimmedString(value) || "#3B82F6";
}

function normalizeDeviceRoutingMode(value: unknown): "default" | "keywords" {
  const normalized = asTrimmedString(value);
  return normalized === "keywords" ? "keywords" : "default";
}

function normalizeDeviceStageType(value: unknown) {
  return normalizeStationType(value);
}

function normalizeDeviceDisplayOrder(value: unknown, fallback = 0) {
  return asNullableNumber(value) ?? fallback;
}

function normalizeDeviceRoutingKeywords(value: unknown): string[] {
  return asStringArray(value)
    .map((keyword) => keyword.toLowerCase())
    .filter(Boolean);
}

function normalizeDeviceNextIds(value: unknown, fallback?: unknown): string[] {
...
function resolveKdsDeviceRow(
  row: DocRow,
  stationMap: Map<string, Record<string, unknown>>
): ResolvedKdsDevice {
  ...
  const displayOrder =
    normalizeDeviceDisplayOrder(row.data.display_order, stationId ? asNumber(stationMap.get(stationId)?.sort_order, 0) : 0);
  ...
  return {
    ...
    is_entry_device:
      typeof row.data.is_entry_device === "boolean"
...
function isResolvedDeviceOnline(device: ResolvedKdsDevice) {
...
function buildItemRoutingText(params: {
  ...
  subExtras?: Array<Record<string, unknown>>;
}) {
  ...
  const explicitBorder = [...(params.extras ?? []), ...(params.subExtras ?? [])].some(
    (extra) => asTrimmedString(extra.kds_category) === "border"
  );

  return `${asTrimmedString(params.notes)} ${extrasText} ${subExtrasText} ${
    explicitBorder ? "border borda" : ""
  }`
    .toLowerCase()
    .trim();
}

function filterEntryDevicesForItem(devices: ResolvedKdsDevice[], itemRoutingText: string) {
  const entryDevices = devices.filter((device) => device.is_entry_device);
  if (!entryDevices.length) return [] as ResolvedKdsDevice[];
...
function getFallbackDevicesFromEntryNextSteps(
  allDevices: ResolvedKdsDevice[],
  onlineDevices: ResolvedKdsDevice[]
) {
  ...
  if (!fallbackNextIds.length) return [] as ResolvedKdsDevice[];

  const allFallbackDevices = fallbackNextIds
    .map((deviceId) => allDevices.find((device) => device.device_id === deviceId) ?? null)
    .filter((device): device is ResolvedKdsDevice => !!device);
  if (!allFallbackDevices.length) return [] as ResolvedKdsDevice[];
...
function pickLeastLoadedDeviceId(
  devices: ResolvedKdsDevice[],
...
function getCandidateDevicesForStations(
  stations: Array<Record<string, unknown> & { id: string }>,
  devicesByStation: Map<string, ResolvedKdsDevice[]>,
  onlineDevicesByStation: Map<string, ResolvedKdsDevice[]>
) {
...
function isActiveKdsItemForLoad(data: Record<string, unknown>) {
  if (asNullableString(data.cancelled_at) || asNullableString(data.served_at)) return false;
  const itemStatus = asTrimmedString(data.status);
  if (itemStatus === "cancelled" || itemStatus === "delivered") return false;
  const stationStatus = asTrimmedString(data.station_status);
  return stationStatus === "waiting" || stationStatus === "in_progress";
}

async function getDeviceLoadCounts(
  tenantRef: FirebaseFirestore.DocumentReference<FirebaseFirestore.DocumentData>,
  deviceIds: string[],
  ...
  for (const chunk of chunkStrings(deviceIds)) {
    ...
    snapshot.docs.forEach((docSnap) => {
      if (excludeItemId && docSnap.id === excludeItemId) return;
      const data = (docSnap.data() ?? {}) as Record<string, unknown>;
      if (!isActiveKdsItemForLoad(data)) return;
...
async function getPreferredOrderDeviceId(
  tenantRef: FirebaseFirestore.DocumentReference<FirebaseFirestore.DocumentData>,
  orderId: string,
  ...
  for (const docSnap of snapshot.docs) {
    if (excludeItemId && docSnap.id === excludeItemId) continue;
    const data = (docSnap.data() ?? {}) as Record<string, unknown>;
    if (!isActiveKdsItemForLoad(data)) continue;
...
function sortStationsByOrder<T extends KdsStationLike>(stations: T[]): T[] {
  return [...stations]
    .filter((station) => station.is_active !== false)
    .sort((left, right) => asNumber(left.sort_order) - asNumber(right.sort_order));
}

function getStationsAtFirstOrder<T extends KdsStationLike>(stations: T[]): T[] {
...
function getEntryStationsForItem<T extends KdsStationLike>(
  stations: T[],
  hasBorder: boolean
): T[] {
  ...
  if (!nonTerminalStations.length) return [];
...
function getNextStationsInFlow<T extends KdsStationLike>(
  stations: T[],
  ...
  let candidates = sortedStations.filter(
    (station) => station.is_active !== false && asNumber(station.sort_order) > currentOrder
  );
...
function getNextStationInFlow<T extends KdsStationLike>(
  stations: T[],
  currentStationId: string,
  orderType?: string
): T | null {
...
function toStationResponse(row: DocRow) {
  return {
    id: row.id,
    ...row.data,
  };
...
async function listTenantDocsByFieldIn(
  tenantRef: FirebaseFirestore.DocumentReference<FirebaseFirestore.DocumentData>,
  ...
  if (!uniqueValues.length) return [] as DocRow[];
...
async function getRootDocsByIds(collectionName: string, ids: string[]) {
  const uniqueIds = [...new Set(ids.filter(Boolean))];
  if (!uniqueIds.length) return [] as DocRow[];
...
async function getTenantDocsByIds(
  tenantRef: FirebaseFirestore.DocumentReference<FirebaseFirestore.DocumentData>,
  collectionName: string,
  ids: string[]
) {
...
async function getKdsSettingsDoc(tenantRef: FirebaseFirestore.DocumentReference<FirebaseFirestore.DocumentData>) {
  const snapshot = await tenantRef.collection("kds_global_settings").limit(1).get();
  if (snapshot.empty) return null;
  return docRow(snapshot.docs[0]);
}

async function listKdsStationsDocs(tenantRef: FirebaseFirestore.DocumentReference<FirebaseFirestore.DocumentData>) {
...
async function ensureInternalStageForDeviceRow(
  tenantRef: FirebaseFirestore.DocumentReference<FirebaseFirestore.DocumentData>,
  row: DocRow
) {
  if (row.data.is_active === false) return row;
  ...
  const currentStationData =
    currentStationSnap?.exists && !asNullableString(currentStationSnap.data()?.deleted_at)
      ? ((currentStationSnap.data() ?? {}) as Record<string, unknown>)
      : null;
  ...
  let deviceDisplayOrder = normalizeDeviceDisplayOrder(
  ...
  if (!stageSnap.exists || asNullableString(stageSnap.data()?.deleted_at)) {
    ...
    const nextSortOrder =
      stationRows.reduce((highest, stationRow) => Math.max(highest, asNumber(stationRow.data.sort_order, 0)), 0) + 1;
...
async function listKdsDevicesDocs(tenantRef: FirebaseFirestore.DocumentReference<FirebaseFirestore.DocumentData>) {
...
async function countActiveItemsForStation(
  tenantRef: FirebaseFirestore.DocumentReference<FirebaseFirestore.DocumentData>,
  stationId: string
) {
  ...
  return snapshot.docs.reduce((total, docSnap) => {
    ...
    if (asNullableString(row.cancelled_at) || asNullableString(row.served_at)) return total;
    if (stationStatus !== "waiting" && stationStatus !== "in_progress") return total;
    return total + 1;
  }, 0);
}

async function findKdsDeviceAcrossTenants(
  firestore: ReturnType<typeof getFirestore>,
  field: "verification_code" | "auth_code",
  value: string,
  excludePath?: string
) {
  ...
  for (const tenantSnap of tenantsSnapshot.docs) {
    ...
    const match = snapshot.docs.find((docSnap) => {
      if (docSnap.ref.path === excludePath) return false;
      return !isDeletedKdsDeviceData((docSnap.data() ?? {}) as Record<string, unknown>);
    });
    if (match) {
      return match;
    }
...
async function getKdsDeviceByCodes(firestore: ReturnType<typeof getFirestore>, verificationCode: string) {
  return await findKdsDeviceAcrossTenants(firestore, "verification_code", verificationCode);
}

async function getKdsDeviceByPublicId(
  tenantRef: FirebaseFirestore.DocumentReference<FirebaseFirestore.DocumentData>,
  deviceId: string
) {
  const snapshot = await tenantRef.collection("kds_devices").where("device_id", "==", deviceId).limit(1).get();
  if (snapshot.empty) return null;
  const match = snapshot.docs.find((docSnap) => !isDeletedKdsDeviceData((docSnap.data() ?? {}) as Record<string, unknown>));
  return match ?? null;
}

async function generateUniqueKdsCode(
  firestore: ReturnType<typeof getFirestore>,
  field: "verification_code" | "auth_code",
  excludePath?: string
) {
...
async function ensureKdsDeviceCodes(
  firestore: ReturnType<typeof getFirestore>,
  deviceSnap:
    | FirebaseFirestore.QueryDocumentSnapshot<FirebaseFirestore.DocumentData>
    | FirebaseFirestore.DocumentSnapshot<FirebaseFirestore.DocumentData>
...
async function requireKdsDeviceAccess(req: Request, res: Response, tenantId: string, deviceId: string) {
  const firestore = getFirestore();
  const tenantRef = firestore.collection("tenants").doc(tenantId);
...
async function buildKdsOrders(
  tenantRef: FirebaseFirestore.DocumentReference<FirebaseFirestore.DocumentData>,
  statuses?: string[],
  options?: { deviceId?: string }
) {
  const normalizedStatuses = (statuses ?? []).map((status) => status.trim()).filter(Boolean);
  let ordersRaw: Array<Record<string, unknown> & { id: string }> = [];
  let itemsRaw: Array<Record<string, unknown> & { id: string }> = [];
  ...
  if (options?.deviceId) {
    ...
    const itemById = new Map<string, Record<string, unknown> & { id: string }>();
    [...activeItemsSnapshot.docs, ...cancelledItemsSnapshot.docs].forEach((docSnap) => {
      itemById.set(docSnap.id, { id: docSnap.id, ...(docSnap.data() as Record<string, unknown>) });
    });

    itemsRaw = sortByIsoAsc([...itemById.values()]);
    if (!itemsRaw.length) return [];

    const orderRows = await getTenantDocsByIds(
      tenantRef,
      "orders",
      itemsRaw.map((item) => asTrimmedString(item.order_id)).filter(Boolean)
    );

    ordersRaw = sortByIsoDesc(
      orderRows
    ...
    ordersRaw = sortByIsoDesc(
      ordersSnapshot.docs
        .map((snap) => ({ id: snap.id, ...(snap.data() as Record<string, unknown>) }) as Record<string, unknown> & { id: string })
        .filter((order) => !normalizedStatuses.length || normalizedStatuses.includes(asTrimmedString(order.status)))
    );
    ...
    itemsRaw = sortByIsoAsc(
      itemRows.map((row) => ({ id: row.id, ...(row.data as Record<string, unknown>) }))
    );
  }
  ...
  const [extrasRows, subItemRows, tablesRows, productsRows, variationsRows, profilesRows, stationRows, settingsRow, deviceRows] = await Promise.all([
    ...
    getTenantDocsByIds(
      tenantRef,
      "tables",
      ordersRaw.map((order) => asTrimmedString(order.table_id)).filter(Boolean)
    ),
    getTenantDocsByIds(
      tenantRef,
      "products",
      itemsRaw.map((item) => asTrimmedString(item.product_id)).filter(Boolean)
    ),
    getTenantDocsByIds(
      tenantRef,
      "product_variations",
      itemsRaw.map((item) => asTrimmedString(item.variation_id)).filter(Boolean)
    ),
    getRootDocsByIds(
      "profiles",
      [
  ...
  const subExtraRows = await listTenantDocsByFieldIn(
    tenantRef,
    "order_item_sub_item_extras",
    "sub_item_id",
    subItemRows.map((subItem) => subItem.id)
  );
  ...
  const activeStations: Array<Record<string, unknown> & { id: string }> = sortStationsByOrder(
    stationRows
    .map((row) => ({ id: row.id, ...(row.data as Record<string, unknown>) }) as Record<string, unknown> & { id: string })
    .filter((station) => station.is_active !== false)
  );
  ...
  const allDevicesByStation = new Map<string, ResolvedKdsDevice[]>();
  const onlineDevicesByStation = new Map<string, ResolvedKdsDevice[]>();
  const deviceByPublicId = new Map<string, ResolvedKdsDevice>();
  ...
  const extrasByItemId = new Map<string, Array<Record<string, unknown>>>();
  ...
  const subExtrasBySubItemId = new Map<string, Array<Record<string, unknown>>>();
  ...
  const subItemsByItemId = new Map<string, Array<Record<string, unknown>>>();
  const subItemIdsByItemId = new Map<string, string[]>();
  const subItemsRaw: Array<Record<string, unknown> & { id: string }> = sortByIsoAsc(
    subItemRows.map((row) => ({ id: row.id, ...(row.data as Record<string, unknown>) }))
  );
  ...
  const deviceCardLoadCounts = new Map<string, number>();
  ...
  itemsRaw.forEach((item) => {
    ...
    if (!stationId || !deviceId) return;
    if (asNullableString(item.cancelled_at) || asNullableString(item.served_at)) return;
    if (stationStatus !== "waiting" && stationStatus !== "in_progress") return;
    if (!deviceCardLoadCounts.has(deviceId)) return;
    deviceCardLoadCounts.set(deviceId, (deviceCardLoadCounts.get(deviceId) ?? 0) + 1);
  });

  const itemRoutingTextByItemId = new Map<string, string>();
  itemsRaw.forEach((item) => {
    ...
    itemRoutingTextByItemId.set(
      item.id,
      buildItemRoutingText({
        notes: item.notes,
        extras: itemExtras,
  ...
  const entryDeviceAssignmentByOrderGroup = new Map<string, string>();

  const pickAssignmentForCandidateStations = (
    candidateStations: Array<Record<string, unknown> & { id: string }>,
    preferredDeviceId?: string | null
  ) => {
    const candidateDevices = getCandidateDevicesForStations(
      candidateStations,
      allDevicesByStation,
      onlineDevicesByStation
    );
    if (!candidateDevices.length) return null;
    ...
    if (!selectedDevice?.station_id) return null;
  ...
  const getInitialAssignmentForItem = (
    item: Record<string, unknown> & { id: string },
    orderId: string,
    orderStatus: string
  ) => {
    if (orderStatus === "ready" || orderStatus === "delivered" || orderStatus === "cancelled") return null;
    if (asNullableString(item.cancelled_at) || asNullableString(item.served_at)) return null;
    if (asTrimmedString(item.status) === "delivered" || asTrimmedString(item.status) === "cancelled") return null;
    if (asTrimmedString(item.station_status) === "completed" || asTrimmedString(item.station_status) === "done") return null;

    const itemRoutingText = itemRoutingTextByItemId.get(item.id) ?? "";
    const candidateEntryStations = getEntryStationsForItem(
      activeStations,
    ...
    if (!candidateEntryDevices.length) return null;
    ...
    const preferredDeviceId =
      existingEntryDeviceId && candidateEntryDevices.some((device) => device.device_id === existingEntryDeviceId)
  ...
  const canAssignItemToKds = (item: Record<string, unknown>, status: string) => {
    if (asNullableString(item.cancelled_at) || asNullableString(item.served_at)) return false;
    if (status === "cancelled" || status === "delivered") return false;
    const stationStatus = asTrimmedString(item.station_status);
    if (stationStatus === "completed" || stationStatus === "done") return false;
    return true;
  };

  type ResolvedKdsItem = (Record<string, unknown> & { id: string }) & {
    order_id: string;
    normalized_item_status: string;
    resolved_station_id: string | null;
    resolved_station_status: string | null;
    resolved_device_id: string | null;
    preferred_device_id: string | null;
    can_assign_to_device: boolean;
  };

  const resolvedItems: ResolvedKdsItem[] = itemsRaw.map((item) => {
    ...
    const shouldReassignWithinCurrentStation =
      !!originalStation &&
    ...
    const reboundAssignment = shouldReassignWithinCurrentStation
      ? pickAssignmentForCandidateStations(
          activeStations.filter((station) => station.id === originalStationId),
          originalDeviceId
        )
      : null;
  ...
  resolvedItems.forEach((item) => {
    ...
    assignmentRepairBatch.set(
      tenantRef.collection("order_items").doc(item.id),
      {
  ...
  return ordersRaw.map((order) => {
    ...
    const orderItems = (itemsByOrderId.get(order.id) ?? []).filter((item) => {
      if (!options?.deviceId) return true;
...
async function createKdsStationLogEntry(
  tenantRef: FirebaseFirestore.DocumentReference<FirebaseFirestore.DocumentData>,
  tenantId: string,
  payload: {
...
export const kdsDeviceAuth = onRequest({ region: "us-central1" }, async (req, res) => {
  ...
  try {
    ...
    if (action === "regenerate_codes" || action === "get_codes") {
      ...
      if (isDeletedKdsDeviceData((deviceSnap.data() ?? {}) as Record<string, unknown>)) {
        json(res, 404, { error: "Dispositivo nao encontrado" });
        return;
      }
...
export const kdsData = onRequest({ region: "us-central1" }, async (req, res) => {
  if (handlePreflight(req, res)) return;
  ...
  try {
    if (stationAdminActions.has(action)) {
      ...
      if (action === "set_device_station") {
        ...
        if (!deviceSnap.exists || isDeletedKdsDeviceData((deviceSnap.data() ?? {}) as Record<string, unknown>)) {
          json(res, 404, { error: "Dispositivo nao encontrado" });
          return;
        }
        ...
        if (stationId) {
          ...
          const stationData = (stationSnap.data() ?? {}) as Record<string, unknown>;
        ...
        json(res, 200, {
          success: true,
          device: sanitizeKdsDevice(updatedSnap.id, tenantId, (updatedSnap.data() ?? {}) as Record<string, unknown>),
        });
        return;
      }
      ...
      if (action === "delete_station") {
        ...
        const activeItemDocs = activeItemsSnap.docs.filter((docSnap) => {
          const row = (docSnap.data() ?? {}) as Record<string, unknown>;
          const stationStatus = asTrimmedString(row.station_status);
          if (asNullableString(row.cancelled_at) || asNullableString(row.served_at)) return false;
          return stationStatus === "waiting" || stationStatus === "in_progress";
        });
        ...
        const stationLoadCounts = new Map<string, number>();
        ...
        const pickReplacementDeviceId = (replacementStationId: string) => {
        ...
        assignedDevicesSnap.docs.forEach((deviceSnap) => {
          const deviceData = (deviceSnap.data() ?? {}) as Record<string, unknown>;
          if (isDeletedKdsDeviceData(deviceData)) return;
          batch.set(
            deviceSnap.ref,
            {
              station_id: null,
    ...
    if (action === "get_orders" || action === "get_all") {
      ...
      const [orders, settings, stations] = await Promise.all([
        ...
        action === "get_all" ? listKdsStationsDocs(access.tenantRef) : Promise.resolve([] as DocRow[]),
      ]);
      ...
      json(res, 200, {
        orders,
        device: sanitizeKdsDevice(
          access.deviceSnap.id,
          tenantId,
          (access.deviceSnap.data() ?? {}) as Record<string, unknown>
        ),
    ...
    if (action === "get_station_history") {
      const stationId =
        asTrimmedString(req.body?.station_id) ||
      ...
      const orderItemIds = Array.from(
        new Set(orderedLogs.map((entry) => asTrimmedString(entry.order_item_id)).filter(Boolean))
      );

      const orderItemSnaps = await Promise.all(
        orderItemIds.map((orderItemId) => access.tenantRef.collection("order_items").doc(orderItemId).get())
      );
      const orderItems = orderItemSnaps
        .filter((snap) => snap.exists)
        .map((snap) => ({ id: snap.id, ...(snap.data() ?? {}) } as Record<string, unknown> & { id: string }));
      const orderItemMap = new Map(orderItems.map((item) => [item.id, item]));

      const orderIds = Array.from(
        new Set(orderItems.map((item) => asTrimmedString(item.order_id)).filter(Boolean))
      );
      const productIds = Array.from(
        new Set(orderItems.map((item) => asTrimmedString(item.product_id)).filter(Boolean))
      );
      const variationIds = Array.from(
        new Set(orderItems.map((item) => asTrimmedString(item.variation_id)).filter(Boolean))
      );

      const orderSnaps = await Promise.all(
        orderIds.map((orderId) => access.tenantRef.collection("orders").doc(orderId).get())
      );
      ...
      const productSnaps = await Promise.all(
        productIds.map((productId) => access.tenantRef.collection("products").doc(productId).get())
      );
      ...
      const variationSnaps = await Promise.all(
        variationIds.map((variationId) => access.tenantRef.collection("product_variations").doc(variationId).get())
      );
      ...
      const tableSnaps = await Promise.all(
        tableIds.map((tableId) => access.tenantRef.collection("tables").doc(tableId).get())
      );
    ...
    if (action === "claim_item") {
      ...
      try {
        await access.firestore.runTransaction(async (transaction) => {
          ...
          const canRebindToCurrentDeviceStation =
            !!deviceStationId &&
            stationStatus === "waiting" &&
          ...
          const alreadyClaimedByThisDevice =
    ...
    if (action === "update_item_station") {
      ...
      if (req.body?.station_id !== undefined) updates.current_station_id = stationId;
      if (req.body?.station_status !== undefined) updates.station_status = stationStatus;
      ...
      if (stationStatus === "completed") updates.station_completed_at = now;
      if (stationStatus !== "in_progress") {
    ...
    if (action === "finalize_order_from_status") {
      ...
      const activeStations: Array<Record<string, unknown> & { id: string }> = stationRows
        .map((row) => ({ id: row.id, ...row.data }) as Record<string, unknown> & { id: string })
        .filter((station) => station.is_active !== false)
        .sort((left, right) => asNumber(left.sort_order) - asNumber(right.sort_order));
      ...
      if (orderType === "dine_in" && currentStationId) {
        ...
        const nextOrderStatusStation = currentStation
          ? activeStations
              .filter((station) =>
                asTrimmedString(station.station_type) === "order_status" &&
        ...
        if (nextOrderStatusStation) {
          ...
          const nextStationDevices = resolvedDevices.filter(
            (device) => device.station_id === nextOrderStatusStation.id
          );
    ...
    if (action === "log_station") {
      ...
      const stationId = asTrimmedString(req.body?.station_id);
      const logAction = asTrimmedString(req.body?.action) as "entered" | "started" | "completed" | "skipped";
    ...
    if (action === "smart_move_item") {
      ...
      const effectiveCurrentDeviceId =
        currentDeviceId && currentDeviceId !== deviceId && claimedByDeviceId === deviceId
          ? deviceId
          : currentDeviceId;
      ...
      const devicesByStation = new Map<string, ResolvedKdsDevice[]>();
      const onlineDevicesByStation = new Map<string, ResolvedKdsDevice[]>();
      ...
      let targetStation: (Record<string, unknown> & { id: string }) | null = null;

      if (!isTerminalStage) {
        const candidateNextDevices = getCandidateDevicesForStations(
          nextStations,
          devicesByStation,
          onlineDevicesByStation
        );
        ...
        const selectedNextDeviceId = pickLeastLoadedDeviceId(
          candidateNextDevices,
          deviceLoadCounts,
          preferredDeviceId
        );
      ...
      if (!targetStationId || asTrimmedString(targetStation?.station_type) === "order_status") {
        if (orderId && orderSnap?.exists) {
          const orderItemsSnapshot = await access.tenantRef.collection("order_items").where("order_id", "==", orderId).get();
          const orderStatusStationIds = new Set(
            activeStations
          ...
          const allItemsReady = orderItemsSnapshot.docs.every((docSnap) => {
            const row = (docSnap.data() ?? {}) as Record<string, unknown>;
            const currentItemStationId =
              docSnap.id === itemId ? targetStationId : asNullableString(row.current_station_id);
            const currentItemStationStatus =
              docSnap.id === itemId ? (targetStationId ? "waiting" : "done") : asTrimmedString(row.station_status);

...
export { kdsData, kdsDeviceAuth } from "./kds";
...
async function deleteOrphanTenantArtifacts(
...
async function isCallerOwnedBootstrapShell(
  tenantSnap: FirebaseFirestore.QueryDocumentSnapshot<FirebaseFirestore.DocumentData>,
  callerUid: string
) {
  ...
  for (const subcollection of subcollections) {
    ...
    const limit = BOOTSTRAP_TENANT_COLLECTION_LIMITS[subcollection.id as keyof typeof BOOTSTRAP_TENANT_COLLECTION_LIMITS] + 1;
    ...
    if (subcollection.id === "tenant_members") {
      ...
      const member = snapshot.docs[0].data() as Record<string, unknown>;
    ...
    if (subcollection.id === "user_roles") {
      ...
      const role = snapshot.docs[0].data() as Record<string, unknown>;
...
async function resolveTenantSlugAvailability(
  firestore: ReturnType<typeof getFirestore>,
  slug: string,
  callerUid: string
) {
  ...
  if (matchingTenants.empty) {
    return { available: true, cleanedTenantIds: [] as string[] };
  }
  ...
  for (const tenantSnap of matchingTenants.docs) {
    ...
    const createdAt =
      typeof tenantData.created_at === "string" ? new Date(tenantData.created_at).getTime() : Number.NaN;
...
function proxyToSupabase(edgeFunctionName: string) {
  return onRequest({ region: "us-central1" }, async (req, res) => {
    if (handlePreflight(req, res)) return;
...
export const health = onRequest({ region: "us-central1" }, async (req, res) => {
  if (handlePreflight(req, res)) return;
...
export const checkTenantSlugAvailability = onRequest({ region: "us-central1" }, async (req, res) => {
  if (handlePreflight(req, res)) return;
...
export const tenantInvitationInfo = onRequest({ region: "us-central1" }, async (req, res) => {
  if (handlePreflight(req, res)) return;
  ...
  try {
    ...
    const invitation = invitationSnap.docs[0].data() as InvitationDoc;
...
export const acceptTenantInvitation = onRequest({ region: "us-central1" }, async (req, res) => {
  if (handlePreflight(req, res)) return;
...
export const platformAdminCreateAdmin = onRequest({ region: "us-central1" }, async (req, res) => {
  if (handlePreflight(req, res)) return;
...
export const platformAdminDeleteAdmin = onRequest({ region: "us-central1" }, async (req, res) => {
  if (handlePreflight(req, res)) return;
  ...
  try {
    ...
    const admin = adminSnap.data() as { user_id?: string | null; email?: string | null };
    const callerEmail = typeof caller.email === "string" ? caller.email.toLowerCase().trim() : "";
...
export const platformAdminUpdateTenantStatus = onRequest({ region: "us-central1" }, async (req, res) => {
  if (handlePreflight(req, res)) return;
...
export const bootstrapTenant = onRequest({ region: "us-central1" }, async (req, res) => {
  if (handlePreflight(req, res)) return;

  const caller = await requireAuth(req, res);
  if (!caller) return;

  const { name, slug } = (req.body ?? {}) as Partial<BootstrapTenantPayload>;
...
export const createUser = onRequest({ region: "us-central1" }, async (req, res) => {
  ...
  const { email, password, name, role, tenant_id } = (req.body ?? {}) as {
    email?: string;
    password?: string;
    name?: string;
    role?: string;
  ...
  try {
    ...
    await getAuth().setCustomUserClaims(created.uid, {
      role,
...
export const adminUpdateUser = onRequest({ region: "us-central1" }, async (req, res) => {
  if (handlePreflight(req, res)) return;
  const caller = await requireAuth(req, res);
  if (!caller) return;

  const { userId, email, password, name } = (req.body ?? {}) as {
    userId?: string;
    email?: string;
    password?: string;
    name?: string;
  ...
  if (name) updates.displayName = name.toUpperCase();
...
export const adminDeleteUser = onRequest({ region: "us-central1" }, async (req, res) => {
  if (handlePreflight(req, res)) return;
  const caller = await requireAuth(req, res);
  if (!caller) return;
...
export const deleteUser = onRequest({ region: "us-central1" }, async (req, res) => {
  if (handlePreflight(req, res)) return;
  const caller = await requireAuth(req, res);
  if (!caller) return;
...
export const qzSign = onRequest({ region: "us-central1" }, async (req, res) => {
  if (handlePreflight(req, res)) return;
  const caller = await requireAuth(req, res);
  if (!caller) return;
...
export const cardapiowebSyncStatus = proxyToSupabase("cardapioweb-sync-status");
export const cardapiowebSyncOrders = proxyToSupabase("cardapioweb-sync-orders");
export const cardapiowebWebhook = proxyToSupabase("cardapioweb-webhook");
export const replicateMenu = proxyToSupabase("replicate-menu");
export const sendReopenNotification = proxyToSupabase("send-reopen-notification");
export const openaiTts = proxyToSupabase("openai-tts");
export const elevenlabsTts = proxyToSupabase("elevenlabs-tts");
export const productionApi = proxyToSupabase("production-api");
export const productionWebhook = proxyToSupabase("production-webhook");
export const publicStore = proxyToSupabase("public-store");

    ...
    if (action === 'menu') {
      ...
      if (groupIds.length > 0) {
        const { data: groups } = await supabase
          ...
          .select('id, name, description, selection_type, is_required, min_selections, max_selections, sort_order, price_calculation_type, channels, visibility')
          .eq('tenant_id', tenant.id)
          .eq('is_active', true)
          .in('id', groupIds)
          .order('sort_order')
        ...
        const { data: gOptions } = await supabase
        ...
        const optionIds = [...new Set(groupOptions.map(go => go.option_id))]
